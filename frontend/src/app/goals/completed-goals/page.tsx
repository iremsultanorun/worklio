...
export default function CompletedGoalsPage() {
  ...
  const fetchGoals = useCallback(async () => {
    ...
    setGoals(data.filter(g => g.is_completed === 1))
  }, [])
  ...
  return (
    <div className="p-4 flex flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Tamamlanan Hedefler</h1>
          <p className="text-sm text-gray-500">{goals.length} hedef</p>
        </div>
      </div>
      <GoalsList goals={goals} type="completed" onRefresh={fetchGoals} />
    </div>
  )
}